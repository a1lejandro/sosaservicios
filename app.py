# SOSA SERVICIO v8.3 WEB - SEGURO SIN MOSTRAR CLAVE
from flask import Flask, render_template_string, request, session
import json, os
from datetime import datetime

app = Flask(__name__)
app.secret_key = "sosa-2026-final-seguro"
...
<h2 style="text-align:center;margin:5px">{{negocio}} v8.3 WEB</h2>
<div class="alias">ALIAS PARA COBROS: {{alias}} | Transferencia / Efectivo</div>
...
<input name="q" placeholder="Buscar codigo o nombre" value="{{q}}" style="width:60%">
...
<input name="codigo" placeholder="Codigo SOS-..." style="width:14%" required>
...
<input name="ubicacion" placeholder="Ubic" style="width:8%">
<input name="clave" placeholder="Clave admin" type="password" style="width:12%" required>
<button class="btn" style="background:#4CAF50;color:white">Agregar</button>
</form>

<div style="max-height:300px;overflow:auto;margin-top:8px">
{% for p in productos %}
<div class="prod {{'rojo' if p.cant==0 else 'azul' if p.cant<=3 else 'verde'}}">
<div style="text-align:left"><b>{{p.codigo}}</b> - {{p.nombre}} | Cant:<b>{{p.cant}}</b> | ${{p.precio}}</div>
<div>
...
<form method="post" style="display:inline"><input type="hidden" name="accion" value="borrar_prod"><input type="hidden" name="idx" value="{{p.idx}}"><input name="clave" type="password" placeholder="clave" style="width:55px" required><button class="btn" style="background:#f44336;color:white">Borrar</button></form>
</div>
</div>
{% endfor %}
</div>
...
<button name="pago" value="1" class="btn" style="background:#2196F3;color:white;width:48%;padding:12px">VENDER EFECTIVO</button>
<button name="pago" value="2" class="btn" style="background:#22c55e;color:white;width:48%;padding:12px">VENDER TRANSFERENCIA</button>
</form>
...
<td><form method="post"><input type="hidden" name="accion" value="anular_venta"><input type="hidden" name="v_idx" value="{{v.idx_real}}"><input name="clave" type="password" placeholder="clave" style="width:60px" required><button class="btn" style="background:#f44336;color:white">Anular</button></form></td></tr>
{% endfor %}
</table>
</div>
...
<h3>Cierre Hoy: {{hoy}}</h3>
<p>Ventas: {{cierre.cant}} | TOTAL: ${{cierre.total}} | EF: ${{cierre.ef}} | TRANSF: ${{cierre.tr}}</p>
...
def index():
        ...
        if acc=='agregar_prod':
                ...
                for it in cart:
                    for p in productos:
                        if p['codigo']==it['codigo']:
                            ...
                            break
                guardar(ARCH_PROD, productos); guardar(ARCH_VENTAS, ventas); session['carrito']=[]
                mensaje=f"VENTA {medio} ${total} OK - Stock descontado!"
            tab=1
            ...
            if clave!=CLAVE_ADMIN: mensaje="Clave incorrecta"
                ...
                if 0<=v_idx<len(ventas):
                    ...
                    del ventas[v_idx]; guardar(ARCH_PROD, productos); guardar(ARCH_VENTAS, ventas); mensaje="Venta anulada y stock devuelto"
            tab=2
        elif acc=='add_deuda':
            if clave!=CLAVE_ADMIN: mensaje="Clave incorrecta"
    ...
    for v in ventas:
        if hoy in v.get('fecha',''):
            m=get_precio_int(v); tot+=m
            if "EFECTIVO" in v.get('pago',''): tot_ef+=m
            else: tot_tr+=m
            cant+=1; det+=f"{v['fecha']} {v['cliente']} {v['producto']} ${v['precio']} [{v.get('pago','')}]\n"
